...
static nrf_saadc_value_t                adc_buf[SAMPLES_IN_BUFFER];                 /**< Data buffer saadc. */
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            //NRF_GPIO->OUT = (1<<3);               // Pin for enabling power to sensor.
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            //APP_ERROR_CHECK(err_code);
            nrf_gpio_pin_toggle(LED_1);
            //NRF_GPIO->OUT = (0<<3);               // Pin for disabling power to sensor.
            break;
...
{
    ble_conn_state_on_ble_evt(p_ble_evt);    // Forward BLE events to the Connection State module (Pairing/Bonding).
    ...
    pm_ble_evt_handler(p_ble_evt);           // Forward BLE events to the Peer Manager (Pairing/Bonding).
...
void saadc_bat_event_handler(nrf_drv_saadc_evt_t const * p_event) {
...
{   
    ...
    /*nrf_saadc_channel_config_t config_bat = {
        ...
        .acq_time   = NRF_SAADC_ACQTIME_40US,              
        .mode       = NRF_SAADC_MODE_SINGLE_ENDED,         
        .pin_p      = NRF_SAADC_INPUT_VDD,                 // P0.02 Inngang SAADC
        .pin_n      = NRF_SAADC_INPUT_DISABLED
    };*/
    ...
    /*nrf_drv_saadc_config_t config_bat_init = {
        .resolution = NRF_SAADC_RESOLUTION_8BIT,
        .oversample = NRF_SAADC_OVERSAMPLE_DISABLED,
        .interrupt_priority = NRF_APP_PRIORITY_LOW 
    };*/
    ...
    /*err_code = nrf_drv_saadc_init(&config_bat_init, saadc_bat_event_handler);
    APP_ERROR_CHECK(err_code);*/
...
/*uint8_t battery_level_get(void) {
    ...
    uint32_t err_code = nrf_drv_saadc_sample_convert(1,adc_buf_bat);
    APP_ERROR_CHECK(err_code);
